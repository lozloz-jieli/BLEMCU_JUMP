...
{
   u32  totalNo=0;
   totalNo =(HALL_2_COUNTER+HALL_2_COUNTER+HALL_3_COUNTER)/3;
	   HALL_1_COUNTER=0;
	 HALL_2_COUNTER=0;
	 HALL_3_COUNTER=0;
...
static void port_wakeup_callback(u8 index, u8 gpio)
{
	...
	#if TCFG_SKIP_ROPE_ENABLE
     switch (gpio) {
        case TCFG_HALL1_IO_ONE_PORT://hall NO1 port
             if(HALL_1_COUNTER==0)   //  start counter ,if 
			 	{
			 	log_info("Hall_timer_id: %d", Hall_timer_id);
...
//  if interval time between two enges overtime (1.2s) ,make a update date to display and BLE
