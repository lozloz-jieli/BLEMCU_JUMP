...
static void board_devices_init(void)
{
   ...
   /// hall 中断初始化
    ...
    gpio_set_pull_down(TCFG_HALL2_IO_ONE_PORT, 0);
	 ...
	 gpio_set_direction(TCFG_HALL3_IO_ONE_PORT, 1);
    gpio_set_die(TCFG_HALL3_IO_ONE_PORT, 1);
    gpio_set_dieh(TCFG_HALL3_IO_ONE_PORT, 1);
    gpio_set_hd(TCFG_HALL3_IO_ONE_PORT, 0);
    gpio_set_hd0(TCFG_HALL3_IO_ONE_PORT, 0);
	 gpio_set_pull_up(TCFG_HALL3_IO_ONE_PORT ,1);
    gpio_set_pull_down(TCFG_HALL3_IO_ONE_PORT, 0);
    ...
    port_protect(port_group, TCFG_IOKEY_POWER_ONE_PORT);
     port_protect(port_group, TCFG_HALL1_IO_ONE_PORT);
      port_protect(port_group, TCFG_HALL2_IO_ONE_PORT);
        port_protect(port_group, TCFG_HALL3_IO_ONE_PORT);
...
struct port_wakeup port4 = {
	...
	.edge               = FALLING_EDGE,                      //唤醒方式选择,可选：上升沿\下降沿
    .both_edge          = 0,
	.iomap              = TCFG_HALL3_IO_ONE_PORT,         //唤醒口选择
    .filter             = PORT_FLT_2ms,
};
