    ...
    case HCI_EVENT_PACKET:
        ...
        case HCI_EVENT_LE_META:
            ...
            case HCI_SUBEVENT_LE_CONNECTION_COMPLETE:
				...
				//extern void DISPLAY_STATE_DATA_UPDATE(u8 state, u8 var);
				   DISPLAY_STATE_DATA_UPDATE(BT_LINK_STATE, 1);
        ...
        case HCI_EVENT_DISCONNECTION_COMPLETE:
            ...
            ble_op_att_send_init(con_handle, 0, 0, 0);
			 DISPLAY_STATE_DATA_UPDATE(BT_LINK_STATE, 0);   // 取消BT 壮态标志
            set_ble_work_state(BLE_ST_DISCONN);
...
static uint16_t att_read_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
 ...
 u32 CURRENT_DATE=0;
 u32 CURRENT_OCLOKE=0;
 u32 CURRENT_DATE_YEAR=0;
 u32  CURRENT_DATE_MMDD=0;

extern u32 CURRENT_SKIP_NUM;
void SET_COUNTDOWN_MODE(u8 mode,u8 DATA_H,u8 DATA_L)
{
  u16 tmp;
   struct sys_event e;
		    ...
		    if(mode)
		  		  e.u.key.event = KEY_EVENT_APP_CMD_TIMEDOWN;  // 1 倒计时开始
			else
				 e.u.key.event = KEY_EVENT_APP_CMD_COUNTDOWN;    //倒计数
    
		    e.u.key.value =DATA_H;
			e.u.key.value <<=8;
		    e.u.key.value +=DATA_L;
			// log_info("key.value = %d\n", att_value_len);
		    e.arg  = (void *)DEVICE_EVENT_FROM_KEY;
	        sys_event_notify(&e);
			
}
void Send_SKIP_ROPE_NUM_BLE(u32 num,u8 mode)
{
	 u8  send_buf[12];
	 //  u8  buffer_size;
	...
	send_buf[0]=mode;
	send_buf[1]=0x06;
	send_buf[2]=(u8)(num>>16);
    send_buf[3]=(u8)(num>>8);
	send_buf[4]=(u8)(num);
	send_buf[5]=(get_vbat_level()<LOW_POWER_WARN_VAL)?1:5;//BAT_STATUS;
	//buffer_size =6;
	 if (app_send_user_data_check(6)) 
	 	{
		app_send_user_data(ATT_CHARACTERISTIC_85c60002_Notify_VALUE_HANDLE, send_buf, 6, ATT_OP_AUTO_READ_CCC);
		}
 ...
 void Report_SKIP_ROPE_MODE(u8 mode,u32 targer_num, u8 state)
{
	...
	send_buf[1]=0x06;
	send_buf[2] =state;
	send_buf[3]=(u8)(targer_num>>16);
    send_buf[4]=(u8)(targer_num>>8);
	send_buf[5]=(get_vbat_level()<LOW_POWER_WARN_VAL)?1:5;//BAT_STATUS;;
...
#endif
        break;
    case ATT_CHARACTERISTIC_85c60001_WRITE_VALUE_HANDLE:
        printf("\n-85c60001_WRIT_rx(%d):", buffer_size);
        printf_buf(buffer, buffer_size);
		switch (buffer[0]) 
			{
		           case INQUIRY_CURRENT_NUM:
		           
						r_printf("INQUIRY_CURRENT_NUM");
						Send_SKIP_ROPE_NUM_BLE(CURRENT_SKIP_NUM,INQUIRY_CURRENT_NUM);
						break;
				   case SET_COUNTDOWN_TIMER://设置跳绳倒计时时间				   
						r_printf("SET_COUNTDOWN_TIMER");
						SET_COUNTDOWN_MODE(1,buffer[1],buffer[2]);
						break;
				  case SET_COUNTDOWN_TOTAL://设置跳绳倒计数个数				  
						r_printf("SET_COUNTDOWN_TOTAL");
						SET_COUNTDOWN_MODE(0,buffer[1],buffer[2]);
						break;
				   case INQUIRY_LAST_SEVEN_DAY_NUM:
				 		
						break;
			      case SET_PRESENT_TIME:  //设置当前时间 YYYYMMDDHHmmss			      
					  r_printf("SET_PRESENT_TIME");
                      //CURRENT_Date=YYYYMMDD
					  //CURRENT_OCLOCK==HHmmss	
					  CURRENT_DATE_YEAR=(u32)(buffer[1]<<8)+buffer[2];
					  CURRENT_DATE_MMDD=(u32)(buffer[3]<<8)+buffer[4];
					
					   
					  CURRENT_OCLOKE =(u32)(buffer[5]<<8)+buffer[6];	
					  // printf("\nCURRENT_OCLOCK(%2d):", CURRENT_OCLOKE);
					   os_time_dly(100);
					   printf("\nCURRENT_DA(%2d):CURRENT_OCLOCK(%2d)", CURRENT_DATE_YEAR,CURRENT_OCLOKE);
					   os_time_dly(10);
						break;
				 case SET_PRESENT_SKIP_NUMS:
...
void send_data_to_app(u16 handle, u8 *data, u16 len, u8 handle_type)
{
	if (app_send_user_data_check(len)) {
		app_send_user_data(ATT_CHARACTERISTIC_85c60002_Notify_VALUE_HANDLE, data, len, ATT_OP_AUTO_READ_CCC);
	   }
...
#endif
    return app_send_user_data(ATT_CHARACTERISTIC_85c60002_Notify_VALUE_HANDLE, data, len, ATT_OP_AUTO_READ_CCC);//ATT_CHARACTERISTIC_85c60002_Notify_VALUE_HANDLE
...
void bt_ble_init(void)
{
   ...
   tmp_ble_addr =bt_get_mac_addr();
	...
	//log_info("ble jimmy addr:%d:%d:%d:%d:%d:%d\n",tmp_ble_addr[0],tmp_ble_addr[2],tmp_ble_addr[3],tmp_ble_addr[4],tmp_ble_addr[5]);
