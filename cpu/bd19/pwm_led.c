...
//    return;
//}
static void _pwm_led_user_define_mode_handle(u8 display)
{
  //  r_printf("%s[%d]", __func__, display);
    _pwm_led_close_irq();
    led_pwm_pre_set();
    switch (display) {
    case PWM_LED_USER_SLOW_ADV:
        _pwm_led_one_flash_display(1, CFG_LED0_LIGHT, CFG_LED1_LIGHT, 5000+100, -1, 100);
        /* _pwm_led_one_flash_display(1, CFG_LED0_LIGHT, CFG_LED1_LIGHT, 5000+300, -1, 300); */
        break;
    case PWM_LED_USER_FAST_ADV:
        _pwm_led_one_flash_display(1, CFG_LED0_LIGHT, CFG_LED1_LIGHT, 2000+200, -1, 200);
        /* _pwm_led_one_flash_display(1, CFG_LED0_LIGHT, CFG_LED1_LIGHT, 2000+500, -1, 500); */
        break;
    case PWM_LED_USER_POWER_OFF:
        _pwm_led_one_flash_display(1, CFG_LED0_LIGHT, CFG_LED1_LIGHT, 300, -1, 100);
        break;
    }
    __this->last_mode = display;
    return;
}
void pwm_led_close(void)
{
    if(__this->init == 1){
        LED_PWM_DISABLE;
        LED_PWM_BREATHE_DISABLE;  //呼吸灯使能位
        ...
        RESET_PWM_CON2;
        RESET_PWM_CON3;
        led_pin_set_disable(__this->led_pin);
    }
}
