...
// jimmy add keypress to power off
#define POWER_OFF_CNT    9

//OS_SEM SKIP_ROPE_COUNT;
u16 g_auto_shutdown_timer = 0;

u32 CURRENT_SKIP_NUM =0;    // NUM
u32 skip_num_start =0;

u32 CURRENT_TTMER_DOWN_SECONDS =60 ; // SECONDS
u32 CURRENT_NUM_DOWN_TOTAL =1000 ; // SECONDS

u32 TARGET_TTMER_DOWN_SECONDS =60 ; // SECONDS
u32 TARGET_NUM_DOWN_TOTAL =1000 ; // SECONDS

static u16 countdown_start_vibrate_id =0;
static u16 countdown_stop_vibrate_id =0;

static u16 countdown_timer_id =0;
static u16  countdown_timer_display_id=0;

static u8 skip_mode=0;

static u8 is_app_active = 0;
static u8 edr_remote_address[6];
static  u32 SevenDayRopeNum[7]={0,0,0,0,0,0,0};
static  u32 CurrentRopeDisplayNum=0;
static  u8  WhatDay=0;


void Report_SKIP_ROPE_MODE(u8 mode, u32 targer_num,  u8 state);


void KEY_MUTI_FUCN_init(int io_port, int fun)
{
   if(fun==1)   //  general io ouput
  {
    gpio_set_pull_down(io_port, 0);
    gpio_set_pull_up(io_port, 0);
    gpio_set_direction(io_port, 0);
...
void update_timer_for_timerdowm()
{
	if(CURRENT_TTMER_DOWN_SECONDS >0)
	CURRENT_TTMER_DOWN_SECONDS--;
	// Report_SKIP_ROPE_MODE(STATE_TIMER_COUTDOWN ,TARGET_TTMER_DOWN_SECONDS, CURRENT_TTMER_DOWN_SECONDS, 2);//0 --quit,1 --enter,2-- being
	DISPLAY_NUM_DATA_UPDATE(CURRENT_TTMER_DOWN_SECONDS, 1);
}

void stop_vibrate()   //  开振动提示
{
  //start_vibrate_OFF;
	if(countdown_stop_vibrate_id)
	{
		sys_hi_timeout_del(countdown_stop_vibrate_id);
	}
	buzzer_close();
	countdown_stop_vibrate_id = 0;
	log_info("stop_vibrate");
}

void start_vibrate()   //  延时开振动提示
{
  // start vibrating  for 300ms
  // add
	if(countdown_start_vibrate_id)
	{ 
		sys_hi_timeout_del(countdown_start_vibrate_id); 
		countdown_start_vibrate_id=0;
	}
	if(skip_mode !=STATE_FREE_MODE)
	{
		if(countdown_stop_vibrate_id == 0){
			countdown_stop_vibrate_id =sys_hi_timeout_add(NULL,stop_vibrate,TIME_VIBRATER_DURATION);
		}
		//	 start_vibrate_ON;  //改成蜂鸣器响应
			buzzer_ring();
		if(skip_mode ==STATE_TIMER_COUTDOWN)
		{
			countdown_timer_display_id =sys_hi_timer_add(NULL, update_timer_for_timerdowm, 1000); // 每秒更新一次
			log_info("start_vibrate,countdown_timer_display_id==%d",countdown_timer_display_id);
			// 记录开始计时的当前跳绳数， 用于统计这一计时周期的内的数
			skip_num_start=0;
			Report_SKIP_ROPE_MODE(SET_INTER_TIMEROUT_MODEL, TARGET_TTMER_DOWN_SECONDS,  1);//0 --quit,1 --enter,2-- being
		}
		else
		{
			if(countdown_timer_display_id&&skip_mode==STATE_NUM_COUTDOWN)
			{	
				sys_hi_timer_del(countdown_timer_display_id);
				countdown_timer_display_id=0;
			}
			//countdown_timer_display_id =sys_hi_timer_add(NULL, update_timer_for_timerdowm, 1000); // 每秒更新一次
			// 记录开始计时的当前跳绳数， 用于统计这一计时周期的内的数
			skip_num_start=0;
			Report_SKIP_ROPE_MODE(SET_INTER_COUNTER_MODEL, TARGET_NUM_DOWN_TOTAL,  1);//0 --quit,1 --enter,2-- being
		}
	}
  
   log_info("start_vibrate");
}
...
#endif
extern void  Send_SKIP_ROPE_NUM_BLE(u32 num,u8 mode);

//extern void DISPLAY_STATE_DATA_UPDATE(u8 state, u8 var);
//extern  void DISPLAY_NUM_DATA_UPDATE(u32 num);
...
//----------------------------------------------------------------------------
//---update to app and display---


 void fresh_rope_date()
{
	r_printf("-------%d--------",CURRENT_SKIP_NUM);
   CURRENT_SKIP_NUM++;
    if(skip_mode==STATE_FREE_MODE)
      {
        DISPLAY_NUM_DATA_UPDATE(CURRENT_SKIP_NUM,0);
		  Send_SKIP_ROPE_NUM_BLE(CURRENT_SKIP_NUM,INQUIRY_CURRENT_NUM);
      }
    else if(skip_mode ==STATE_NUM_COUTDOWN)
        {
         //  当
         CURRENT_NUM_DOWN_TOTAL--;
         skip_num_start++;
         log_info("CURRENT_NUM_DOWN_TOTAL: %d", CURRENT_NUM_DOWN_TOTAL);
         if(CURRENT_NUM_DOWN_TOTAL==0) // 计数结束
            {
            //countdown_start_vibrate_id =sys_hi_timeout_add(NULL,start_vibrate,TIME_DELAY_TO_START);
            //当时候这里改成启动蜂鸣器
            
            DISPLAY_NUM_DATA_UPDATE(CURRENT_NUM_DOWN_TOTAL,0);
            DISPLAY_STATE_DATA_UPDATE(STATE_FREE_MODE, 0);
			 Report_SKIP_ROPE_MODE(SET_COUNTDOWN_TOTAL, TARGET_NUM_DOWN_TOTAL, 0);//0 --quit,1 --enter,2-- being
            skip_mode=STATE_FREE_MODE;
            }
        else
           {
            DISPLAY_NUM_DATA_UPDATE(CURRENT_NUM_DOWN_TOTAL,0);
			 Send_SKIP_ROPE_NUM_BLE(skip_num_start,SET_COUNTDOWN_TOTAL);
        	}
        }
	else   // time
		{
		 skip_num_start++;
		 Send_SKIP_ROPE_NUM_BLE(skip_num_start,SET_COUNTDOWN_TIMER);
		}
   log_info("HALL_valided_COUNTER: %d", CURRENT_SKIP_NUM);
    log_info("skip_mode: %d", skip_mode);
              // display
...
#endif
    buf[0]= (CURRENT_SKIP_NUM>>24);  // msb
    buf[1]= CURRENT_SKIP_NUM>>16;
	buf[2]= CURRENT_SKIP_NUM>>8;
	buf[3]= CURRENT_SKIP_NUM; //lsb
    ret = syscfg_write(CFG_USER_SKIP_NUM, buf, 4);
    //延时300ms，确保BT退出链路断开
...
int get_battery_level(void)
{
    u16 power = 410;
    u8 level = 0;
    power = get_vbat_level();

//    if(power <= 420  && power > 390){
	if(power > 390){
        level = 3;
    }else if(power <= 390 && power > 350){
        level = 2;
    }else if(power <= 350 && power > 320){
        level = 1;
    }else{
        level = 0;
    }
  log_info("BATT POWER AND LEVEL %d: %d ",power,level);
    return level;
}
...
static void app_start()
{
   ...
   DISPLAY_NUM_DATA_UPDATE(CURRENT_SKIP_NUM,0);
    ...
    DISPLAY_STATE_DATA_UPDATE(STATE_FREE_MODE,0);
     DISPLAY_STATE_DATA_UPDATE(BATT_REPORT,get_battery_level());
    //os_ sem_create(&SKIP_ROPE_COUNT, 1);
   // log_info("os_sem_set");
  // fresh_rope_date();
#if TCFG_USER_EDR_ENABLE
    sys_auto_sniff_controle(1, NULL);
#endif /* SIG_MESH_EN */
 //   ret = syscfg_read(CFG_USER_SKIP_NUM, buf, 4);
 // if(ret)
//  	{
 // 	   CURRENT_SKIP_NUM =(u32)(buf[0]<<24)+(u32)(buf[1]<<16)+(u32)(buf[2]<<8)+(u32)(buf[3]);
  //	} 
  	  //   buf[0]= (CURRENT_SKIP_NUM>>24);  // msb
  //  buf[1]= CURRENT_SKIP_NUM>>16;
//	buf[2]= CURRENT_SKIP_NUM>>8;
...
//no bt,to for test
    ...
    sys_key_event_enable();
		...
		//无操作定时软关机
		g_auto_shutdown_timer = sys_timeout_add(NULL, app_set_soft_poweroff, TCFG_HID_AUTO_SHUTDOWN_TIME * 1000);
#endif
...
// 计时结束处理
void time_down_over_deal()
{
	if(countdown_timer_id)
	{
		sys_hi_timeout_del(countdown_timer_id);
		countdown_timer_id=0;
	}
	if(countdown_timer_display_id)
	{
		sys_hi_timer_del(countdown_timer_display_id);
		countdown_timer_display_id=0;
	}
	...
	Report_SKIP_ROPE_MODE(SET_INTER_TIMEROUT_MODEL, TARGET_TTMER_DOWN_SECONDS, 0);//0 --quit,1 --enter,2-- being

	DISPLAY_NUM_DATA_UPDATE(skip_num_start, 0);
	// skip_mode =STATE_FREE_MODE;
	//  DISPLAY_STATE_DATA_UPDATE(skip_mode, 0);
	// return to free mode
}

// 状态切换，显示，定时相关处理
static void   state_prosses(u8 state,u32 num)
{
  	  // delete all related timer, vibrater  clear
//			  	   start_vibrate_OFF;
	if(countdown_stop_vibrate_id)
	{
		sys_hi_timeout_del(countdown_stop_vibrate_id);
	...
	if(state==STATE_FREE_MODE)
	{

	}
	else if(state==STATE_TIMER_COUTDOWN) // an
	{
		countdown_timer_id =sys_hi_timeout_add(NULL, time_down_over_deal, num*1000);
		countdown_start_vibrate_id =sys_hi_timeout_add(NULL,start_vibrate,TIME_DELAY_TO_START);
	}
	else  //counterdown
	{
		...
		DISPLAY_STATE_DATA_UPDATE(state, 0);
		DISPLAY_NUM_DATA_UPDATE(num,state);
}
			  ////
...
static void app_key_event_handler(struct sys_event *event)
{
    ...
    if (event->arg == (void *)DEVICE_EVENT_FROM_KEY) {
		...
		if(key_value == 1){
			g_f_printf("here are the key_value :%d",key_value);
			if(event_type == KEY_EVENT_CLICK){
				r_printf("clean number");
				if(skip_mode !=STATE_FREE_MODE)   //@ it avilable @STATE_FREE_MODE
				return ;
				CURRENT_SKIP_NUM =0;////
				DISPLAY_STATE_DATA_UPDATE(skip_mode, 0);
				DISPLAY_NUM_DATA_UPDATE(CURRENT_SKIP_NUM,skip_mode);
			}
//			else if(event_type == KEY_EVENT_DOUBLE_CLICK){
//				r_printf("buzzer control");
//				static u8 buzzer_flag;
//				if(!buzzer_flag){
//					buzzer_ring();
//					buzzer_flag = 1;
//				}else{
...
//			}
			...
			if(event_type==KEY_EVENT_HALL_COUNTER)	// hall counter Num, update record and update display
			{
			  CURRENT_SKIP_NUM +=key_value;
			  // Send_SKIP_ROPE_NUM_BLE();
			  // display
			}
			
			// jimmy  add off, but ,not into power off ,
			
			else if(event_type==KEY_EVENT_HOLD) //3S POWER OFF && key_value == TCFG_ADKEY_VALUE0   only one butten
			{
				goto_poweroff_flag++;
				if(goto_poweroff_flag>POWER_OFF_CNT)
				{
					goto_poweroff_flag=0;
					// g_var.powr_off_Manu_flag=1;
					app_set_soft_poweroff();
				}
			}
			else if(event_type == KEY_EVENT_CLICK) // MODE SWITCH // FREE_MODE // COUNTDOWN_TIMER// COUNTDOWN_COUNTER//
			{
			  skip_mode++;
			  if(skip_mode>STATE_NUM_COUTDOWN)// MODE_SWITH ,STATUS _MODE
				 skip_mode =STATE_FREE_MODE;\
			
			  log_info("key deal --skip_mode: %d", skip_mode);
			  if(skip_mode==STATE_FREE_MODE)
			  {
				display_Num =CURRENT_SKIP_NUM;
				state_prosses(skip_mode,display_Num);
			
			  }
			  else if(skip_mode==STATE_TIMER_COUTDOWN)
			  {
				 CURRENT_TTMER_DOWN_SECONDS=60;
				TARGET_TTMER_DOWN_SECONDS =60;
				state_prosses(skip_mode,CURRENT_TTMER_DOWN_SECONDS);
			 
			  }
			  else if(skip_mode==STATE_NUM_COUTDOWN)
			  {
				 CURRENT_NUM_DOWN_TOTAL =1000; //// 手动（非app 下命令），倒计数1000
				 TARGET_NUM_DOWN_TOTAL	=1000;
				 state_prosses(skip_mode,CURRENT_NUM_DOWN_TOTAL);
			  }
			  
			}
			else if(event_type == KEY_EVENT_TRIPLE_CLICK) //   RESET COUNTER AT FREEMODE
			{
				g_printf("here are the clean");
				if(skip_mode !=STATE_FREE_MODE)   //@ it avilable @STATE_FREE_MODE
				return ;
				CURRENT_SKIP_NUM =0;////
				DISPLAY_STATE_DATA_UPDATE(skip_mode, 0);
			...
			else if(event_type == KEY_EVENT_APP_CMD_TIMEDOWN)
			{
				g_printf("-----KEY_EVENT_APP_CMD_TIMEDOWN-----");
				skip_mode =STATE_TIMER_COUTDOWN;
				CURRENT_TTMER_DOWN_SECONDS=key_value;
				TARGET_TTMER_DOWN_SECONDS  =CURRENT_TTMER_DOWN_SECONDS;
				//进入计时模式
				state_prosses(skip_mode,CURRENT_TTMER_DOWN_SECONDS);			  
			}
			else if(event_type == KEY_EVENT_APP_CMD_COUNTDOWN)
			{			
				g_printf("-----KEY_EVENT_APP_CMD_COUNTDOWN-----");
				skip_mode =STATE_NUM_COUTDOWN; //进入计数模式
				CURRENT_NUM_DOWN_TOTAL =key_value;
				TARGET_NUM_DOWN_TOTAL  =CURRENT_NUM_DOWN_TOTAL;
				state_prosses(skip_mode,CURRENT_NUM_DOWN_TOTAL);		 
			}
